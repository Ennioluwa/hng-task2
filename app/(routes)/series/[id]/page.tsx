...
const MoviePage = async ({ params: { id } }: { params: { id: string } }) => {
  ...
  const { data: movie } = await axios.get(
    `https://api.themoviedb.org/3/tv/${id}?language=en-US&api_key=d4c3f3ba31798ade4a6c41b6a29bef60`
  );

  const { data: castData } = await axios.get(
    `https://api.themoviedb.org/3/tv/${id}/credits?language=en-US&api_key=d4c3f3ba31798ade4a6c41b6a29bef60`
  );
  ...
  return (
    <div
      className={`${poppins.className} mx-auto container max-w-7xl p-5 xl:pt-20 flex flex-col gap-5`}
    >
      <img
      ...
      <div className="flex justify-between gap-x-10 gap-y-2 flex-wrap">
        <div className="flex flex-wrap gap-3 items-center text-[23px] font-medium text-[#404040] ">
          ...
          <p data-testid="movie-release-date">
            {new Date(movie.first_air_date).toUTCString()}
          </p>
          <span className=" h-[6px] w-[6px] bg-[#404040] rounded-full "></span>
          <p data-testid="movie-runtime">{movie.episode_run_time[0]} mins</p>
